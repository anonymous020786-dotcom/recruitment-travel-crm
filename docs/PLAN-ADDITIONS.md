...
| **B.3** | **Passkey / WebAuthn** | `webauthn_credentials` table (credential id, COSE public key, sign count, transports, aaguid). Registration (attestation — we accept `none`/`packed` and verify), authentication (assertion — verify `clientDataJSON`, `authenticatorData`, RP-id hash, challenge, signature over the COSE key, sign-count regression check). CBOR + COSE parsing written in-house (no ext-sodium beyond what PHP ships; ES256/RS256). **Largest item — its own step, security-reviewed.** |
| **B.x** | **2FA policy** | Admin roles may be *required* to have a second factor (`settings.auth.require_2fa_roles`); a grace period on first login. |
